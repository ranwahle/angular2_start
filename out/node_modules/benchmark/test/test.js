System.registerModule("../../../../node_modules/benchmark/test/test.js", [], function() {
  "use strict";
  var __moduleName = "../../../../node_modules/benchmark/test/test.js";
  ;
  (function(window, undefined) {
    'use strict';
    var load = typeof require == 'function' ? require : window.load;
    var platform = window.platform || load('../vendor/platform.js/platform.js') || window.platform;
    var QUnit = window.QUnit || (window.setTimeout || (window.addEventListener = window.setTimeout = / /), window.QUnit = load('../vendor/qunit/qunit/qunit' + (platform.name == 'Narwhal' ? '-1.8.0' : '') + '.js') || window.QUnit, load('../vendor/qunit-clib/qunit-clib.js'), (window.addEventListener || 0).test && delete window.addEventListener, window.QUnit);
    var Benchmark = window.Benchmark || (Benchmark = load('../benchmark.js') || window.Benchmark, Benchmark.Benchmark || Benchmark);
    ...
    var support = {'descriptors': !!function() {
        try {
          var o = {};
    ...
    (function() {
      ...
      test('compiles using a custom "toString" method', function() {
        ...
        bench.setup.toString = function() {
          return 'var a = 1;';
        };
        bench.fn.toString = function() {
          return 'throw a;';
        };
        bench.teardown.toString = function() {
          return 'a = 2;';
        };
        bench.run();
        var compiled = bench.compiled;
    ...
    (function() {
      var count = 0;
      var tests = {
        'inlined "setup", "fn", and "teardown"': ('if(/ops/.test(this))this._fn=true;'),
    ...
    (function() {
      ...
      test('allows a single source argument', function() {
        ...
        deepEqual(Benchmark.extend({}, source), {
          'x': 1,
          'y': 1
        });
      });
    ...
    (function() {
      ...
      test('doesn\'t use an object\'s `hasOwnProperty` method', function() {
        var object = {'hasOwnProperty': function() {
            return true;
          }};
    ...
    forOwn({
      ...
      (function() {
        ...
        test('uses a shallow clone of the listeners when emitting', function() {
          var event,
              listener2 = function(eventObject) {
                eventObject.listener2 = true;
              },
              object = Constructor();
          object.on('shallowclone', function(eventObject) {
            event = eventObject;
            object.off(event.type, listener2);
          }).on('shallowclone', listener2).emit('shallowclone');
          ok(event.listener2);
        });
        test('emits a custom event object', function() {
          var event = Benchmark.Event('custom'),
              object = Constructor();
          object.on('custom', function(eventObject) {
            eventObject.touched = true;
          });
          object.emit(event);
          ok(event.touched);
        });
    ...
    (function() {
      test('igores abort calls when the suite isn\'t running', function() {
        var fired = false;
        var suite = Benchmark.Suite('suite', {'onAbort': function() {
            fired = true;
          }});
      ...
      asyncTest('emits an abort event when running', function() {
        var fired = false;
        Benchmark.Suite({'onAbort': function() {
            fired = true;
          }}).on('start', function() {
          this.abort();
    ...
    (function() {
      asyncTest('avoids call stack limits', function() {
        ...
        count++;
        for (var i = 0,
            sub = object; i <= count; i++) {
          sub = sub[i] = {};
        }
        try {
          for (var i = 0,
              sub = Benchmark.deepClone(object); sub = sub[i]; i++) {}
          result = --i == count;
        } catch (e) {}
      });
    }());
...
System.get("../../../../node_modules/benchmark/test/test.js" + '');
