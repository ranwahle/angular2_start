System.registerModule("../../../../node_modules/cliui/test/cliui.js", [], function() {
  "use strict";
  var __moduleName = "../../../../node_modules/cliui/test/cliui.js";
  require('chai').should();
  ...
  describe('cliui', function() {
    describe('div', function() {
      ...
      it('evenly divides text across columns if multiple columns are given', function() {
        var ui = cliui({width: 40});
        ui.div({
          text: 'i am a string that should be wrapped',
          width: 15
        }, 'i am a second string that should be wrapped', 'i am a third string that should be wrapped');
        ui.toString().split('\n').forEach(function(row) {
    ...
    describe('padding', function() {
      ...
      it('preserves leading whitespace as padding', function() {
        ...
        var expected = ['     LEADING WHITESPACE', '     with ansi'];
        ui.toString().split('\n').map(function(l) {
          return stripAnsi(l);
        }).should.eql(expected);
      });
    ...
    describe('border', function() {
      it('allows a border to be placed around a div', function() {
        ...
        var expected = ['.------------------.', '| i am a first     |.------------------.', '| string           || i am a second    |', "'------------------'| string           |", "                    '------------------'"];
        ui.toString().split('\n').should.eql(expected);
      });
    });
    describe('wrap', function() {
      it('allows wordwrap to be disabled', function() {
        var ui = cliui({wrap: false});
        ui.div({
          text: 'i am a string',
          padding: [0, 1, 0, 0]
        }, {
    ...
    describe('span', function() {
      ...
      it('appends to prior line appropriately when strings contain ansi escape codes', function() {
        var ui = cliui({width: 40});
        ui.span({
          text: chalk.green('i am a string that will be wrapped'),
          width: 30
        });
        ui.div({
          text: chalk.blue(' [required] [default: 99]'),
          align: 'right'
        });
        var expected = ['i am a string that will be', 'wrapped         [required] [default: 99]'];
        ui.toString().split('\n').map(function(l) {
    ...
    describe('layoutDSL', function() {
      ...
      it('aligns rows appropriately when they contain ansi escape codes', function() {
        var ui = cliui({width: 40});
        ui.div('  <regex>\t  ' + chalk.red('my awesome regex') + '\t  [regex]\n  ' + chalk.blue('<glob>') + '\t  my awesome glob\t  [required]');
        var expected = ['  <regex>  my awesome     [regex]', '           regex', '  <glob>   my awesome     [required]', '           glob'];
        ui.toString().split('\n').map(function(l) {
          return stripAnsi(l);
        }).should.eql(expected);
      });
      it('ignores ansi escape codes when measuring padding', function() {
        var ui = cliui({width: 25});
        ui.div(chalk.blue('  |                      \n  __|   __|  |   |   _ \\ \n  |    |     |   |   __/ \n \\__| _|    \\__,_| \\___| \n                         '));
        var expected = ['  |', '  __|   __|  |   |   _ \\', '  |    |     |   |   __/', ' \\__| _|    \\__,_| \\___|', '                         '];
        ui.toString().split('\n').map(function(l) {
          return stripAnsi(l);
        }).should.eql(expected);
      });
...
System.get("../../../../node_modules/cliui/test/cliui.js" + '');
